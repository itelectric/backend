...
public class CompanyServiceTests {
    ...
    @Test
    @DisplayName("Should throw ConflictException if nuit is already in use on create company")
    void shouldThrowConflictExceptionIfNuitIsAlreadyInUseOnCreateCompany() {
        Contact contact = UserMocksFactory.contactWithIdFactory();
        Address address = UserMocksFactory.addressWithIdFactory();
        User user = UserMocksFactory.companyWithIdFactory(contact, address);
    ...
    @Test
    @DisplayName("Should throw ConflictException if username is already in use on create company")
    void shouldThrowConflictExceptionIfUsernameIsAlreadyInUseOnCreateCompany() {
        Contact contact = UserMocksFactory.contactWithIdFactory();
        Address address = UserMocksFactory.addressWithIdFactory();
        User user = UserMocksFactory.companyWithIdFactory(contact, address);
    ...
    @Test
    @DisplayName("Should save user info with encrypted password on create company")
    void shouldSaveUserInfoWithEncryptedPasswordOnCreateCompany() throws ConflictException {
        Contact contact = UserMocksFactory.contactWithIdFactory();
        Address address = UserMocksFactory.addressWithIdFactory();
        User user = UserMocksFactory.companyWithIdFactory(contact, address);
